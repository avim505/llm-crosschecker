...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_STATUS") {
    Promise.all([extpay.getUser().catch(() => ({ paid: false })), getInstallId()])
      .then(([user, installId]) => {
  ...
  if (message.type === "VERIFY_RESPONSE") {
    Promise.all([extpay.getUser().catch(() => ({ paid: false })), getInstallId()])
      .then(([user, installId]) => {
        return fetch(`${BACKEND_URL}/api/verify`, {
          method: "POST",
          headers: {
...
// ── Warm-up ping — keeps Render server hot on service worker start ────────────
